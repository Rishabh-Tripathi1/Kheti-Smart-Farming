...
const DiseaseForm = () => {
  ...
  const changeData = async () => {
    if(output.title != undefined){
      const data = output.title;
      await axios
      .post(
        "http://127.0.0.1:5000/changedata",
        data
      )
      .then(function (response) {
        console.log(response["data"]["instructions"])
        setPreventdata({"instructions":response["data"]["instructions"]})
      })
      ...
      {output ? (
        ...
        >
          ...
          {/* <div className="mt-4">
            {preventdata ?
            <span className="font-bold">How to use:</span>{" "}
            {preventdata.how_to_use.map((item, index) => {
              return <p key={index}>{item}</p>;
            })}
             :
             {output.how_to_use.map((item, index) => {
